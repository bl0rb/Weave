...
supports three subcommands:
    ...
    weave_config.py check
        ...
        frontend is actually published on, a half-configured federated
        ...
        and a deploy/.env that no longer matches weave.yaml -- values added
        by hand, or values declared since the last render.
    ...
    weave_config.py helm-values
        Writes deploy/charts/weave/values.generated.yaml -- the same
        declarations, shaped for the Helm chart instead of an .env file.
        The chart is the SECOND consumer of weave.yaml, not a second
        configuration: `config.<service>` holds every non-secret setting,
        serialized exactly as `render` writes it, and `secrets.<service>`
        holds only the NAMES of that service's secrets. A secret's value is
        never written here -- not even as a placeholder; in a cluster it
        comes from a Secret the chart only references by name.
...
DEFAULT_HELM_VALUES_FILE = (
    REPO_ROOT / "deploy" / "charts" / "weave" / "values.generated.yaml"
)

HELM_VALUES_HEADER = (
    "# Generated by scripts/weave_config.py helm-values from weave.yaml."
)

HELM_VALUES_PREAMBLE = """\
# DO NOT EDIT BY HAND -- edit weave.yaml and re-generate:
#
#     python scripts/weave_config.py helm-values \\
#         --out deploy/charts/weave/values.generated.yaml
#
# This is the SECOND consumer of weave.yaml, next to `render` (which writes
# deploy/.env for the compose stack). Both read the same declarations, so a
# setting exists in exactly one place: weave.yaml. Nothing below belongs in
# the chart's hand-maintained values.yaml.
#
# `config.<service>` carries every NON-secret setting, already serialized
# exactly as `render` would write it into the .env (json_list stays compact
# JSON, numbers and booleans stay strings) -- the chart quotes them straight
# into a container's env.
#
# `secrets.<service>` carries only the NAMES of that service's secret
# settings. No value, and no placeholder either: what a name resolves to is
# decided in the cluster, by the Secret named in secrets.existingSecret or
# built from secrets.values (see the chart's values.yaml).
#
# The values here are weave.yaml's own defaults. Unlike `render`, this
# command deliberately ignores environment variables that happen to match a
# rendered variable name: a generated chart values file must describe the
# platform, not the machine it was generated on.
#
# Pass this file alongside the chart's own values, e.g.
#
#     helm install weave deploy/charts/weave \\
#         -f deploy/charts/weave/values.generated.yaml
"""


def build_helm_values(items: list[Item]) -> tuple[dict[str, dict[str, str]], dict[str, list[str]]]:
    """Split the parsed items into (config, secrets) as the chart wants them.

    config: {service: {VAR: serialized value}} -- non-secrets only.
    secrets: {service: [VAR, ...]} -- names only, sorted, no duplicates.

    A service that only ever appears as a secret target (redis) still gets an
    empty config block, so a template can index .Values.config.<service>
    without a nil check.
    """
    ...
    for item in items:
        for service, var in item.targets:
            ...
            secrets.setdefault(service, set())
            if item.secret:
                secrets[service].add(var)
            else:
                config[service][var] = serialize(item.literal, item.value_type)

    return config, {service: sorted(names) for service, names in secrets.items()}


def _yaml_scalar(value: str) -> str:
    """A double-quoted YAML scalar. JSON string syntax is a strict subset of
    YAML's double-quoted style, so json.dumps gets the escaping right for the
    values that actually occur here -- json_list values carry `"` themselves,
    which is exactly the case a naive f'"{value}"' would break."""
    return json.dumps(value)


def helm_values(config_path: Path, out_path: Path) -> int:
    try:
        items = load_items(config_path)
    except ConfigError as exc:
        print(f"Fehler in {config_path}: {exc}", file=sys.stderr)
    ...
    config, secrets = build_helm_values(items)

    lines = [HELM_VALUES_HEADER, HELM_VALUES_PREAMBLE.rstrip("\n"), "", "config:"]
    for service in sorted(config):
        ...
        for var in sorted(settings):
            lines.append(f"    {var}: {_yaml_scalar(settings[var])}")

    lines.append("")
    lines.append("secrets:")
    for service in sorted(secrets):
        names = secrets[service]
        if not names:
            lines.append(f"  {service}: []")
            continue
        lines.append(f"  {service}:")
    ...
    n_config = sum(len(v) for v in config.values())
    n_secrets = sum(len(v) for v in secrets.values())
    print(
        f"Geschrieben: {out_path} "
        f"({n_config} Konfigurationswerte, {n_secrets} Geheimnis-Namen, "
        f"{len(config)} Dienste)."
    )
    return 0
...
RENDER_HEADER = "# Generated by scripts/weave_config.py render from weave.yaml."


def _was_rendered(path: Path) -> bool:
    try:
...
def check(
    ...
    # --- 7) is the .env still what weave.yaml would render? -----------------
    # deploy/.env is generated. Anything in it that weave.yaml does not
    # declare got there by hand -- and anything weave.yaml declares that is
    # missing from it means weave.yaml moved on and nobody re-rendered. Both
    # are exactly the drift this file exists to prevent: the stack keeps
    # running on compose defaults, so nothing complains, and the next person
    # to `render` gets a different .env than the one in production.
    # Only for a file `render` wrote (it starts with the header below): a
    # hand-assembled .env -- a test fixture, a one-off experiment -- has no
    # claim to completeness and would drown in noise here.
    declared: set[str] = {var for item in items for _service, var in item.targets}
    if not service_env_overrides and _was_rendered(default_env_file):
        present = set(values_for(default_env_file))
        undeclared = sorted(present - declared)
        if undeclared:
            findings.append(Finding(
                "Von Hand in die .env geschrieben (weave.yaml kennt sie nicht): "
                + ", ".join(undeclared)
                + ". Entweder in weave.yaml deklarieren oder aus der .env entfernen "
                "-- der naechste `render` wuerde sie stillschweigend verlieren."
            ))
        stale = sorted(
            var for item in items if not item.required
            for _service, var in item.targets if var not in present
        )
        if stale:
            findings.append(Finding(
                "In weave.yaml deklariert, aber nicht in der .env (sie ist aelter als "
                "weave.yaml): " + ", ".join(stale)
                + ". Der Stack laeuft derweil auf den Compose-Voreinstellungen -- "
                "`render` einmal neu ausfuehren."
            ))
...
def main(argv: list[str] | None = None) -> int:
    ...
    helm_p = sub.add_parser(
        "helm-values",
        help="values.generated.yaml fuer das Helm-Chart aus weave.yaml erzeugen",
    )
    helm_p.add_argument(
        "--out", type=Path, default=DEFAULT_HELM_VALUES_FILE,
        help=f"Zieldatei (Default: {DEFAULT_HELM_VALUES_FILE})",
    )
    ...
    if args.command == "helm-values":
        return helm_values(args.config, args.out)
