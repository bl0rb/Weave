...
Ein vierter, `bots/research-agent.yaml.example`, zeigt den **Agentenmodus** (`agent.enabled: true`, rollout plan "Schritt 2 -- Tool-Calls und ein Subagent") — ein direkt-LLM-Bot (nie `model.provider: n8n`) konfiguriert einen oder mehrere Subagenten, jeder mit eigener Mission, explizit erlaubten Collections/Filtern, erlaubten Tools (in dieser Version nur `search_knowledge`) und eigenem Such-/Laufzeitbudget. Aktivierung setzt voraus, dass das konfigurierte Modell Tool-Calls unterstützt (`model.supports_tools`, geprüft beim Laden der Bot-Konfiguration — `fake` unterstützt das immer). Mit genau EINEM konfigurierten Subagenten recherchiert dieser über eine gebundene `search_knowledge`-Tool-Aufruf-Schleife (`backend/app/services/agents.py`), bevor das Haupt-Bot-LLM anhand der strukturierten Fakten antwortet. Mit MEHR als einem Subagenten läuft stattdessen die LangGraph-Orchestrator-Worker-Pipeline aus "Agentenmodus (LangGraph)" unten. Vertrag-Ergänzung (`trace.agent`) in [`contracts/internal-chat.md`](../../contracts/internal-chat.md).

### Agentenmodus (LangGraph)

Rollout plan "Schritt 3 -- LangGraph mit mehreren Subagenten" (`backend/app/services/agent_graph.py`, `bots/research-agent.yaml.example`s `hr-support`-Subagent): sobald ein Bot **mehr als einen** Subagenten konfiguriert, ersetzt eine `langgraph.graph.StateGraph`-Pipeline Schritt 2's Single-Subagent-Pfad (der für genau einen konfigurierten Subagenten unverändert bleibt) — ein klassisches Orchestrator-Worker-Muster (siehe [docs.langchain.com/oss/python/langgraph/workflows-agents](https://docs.langchain.com/oss/python/langgraph/workflows-agents)):

1. **`plan`** — das Haupt-Bot-LLM bekommt genau EIN Tool angeboten, `research_area(agent_id, question)`, und nur Subagenten mit einem für diesen Aufrufer nicht-leeren effektiven Scope überhaupt zur Auswahl. Mehrere Tool-Aufrufe = mehrere parallele Teilfragen; eine reine Textantwort ohne Tool-Aufruf ist der "einfache Pfad" — dann recherchiert genau der eine am besten passende Subagent (dieselbe Auswahl wie Schritt 2's `_select_subagent`).
2. **`research`** (`langgraph.types.Send`-Fan-out, `RunnableConfig(max_concurrency=agent.limits.max_parallel)`) — je ein Worker pro Teilfrage, jeder wiederum Schritt 2's eigene `agents.run_subagent`, begrenzt durch sein eigenes `subagent.limits` UND das turn-weite, geteilte `agent.limits.budget_searches`.
3. **`merge`** — Treffer nach `(document_id, chunk_id)` dedupliziert; Teilfragen mit `status` `"failed"`/`"partial"` oder ganz ohne Fakten lösen — solange geteiltes Budget/Gesamt-Zeitlimit/`agent.limits.max_followups` es zulassen — eine weitere Recherche-Runde für genau diese Teilfragen aus; inhaltlich widersprüchliche Fakten (eine günstige, rein lexikalische Heuristik) werden für den Schlussprompt markiert.
4. **`answer`** — bei fehlenden Fakten/Quellen greift exakt derselbe `bot.guard.require_sources`-Guard wie überall sonst; andernfalls schreibt das Haupt-Bot-LLM die Antwort aus den verifizierten Fakten, ergänzt um einen festen, deutschen Lückenhinweis pro Subagent, der am Ende ohne Ergebnis blieb, während andere welche lieferten.

Ein `threading.Event` (dieselbe Cancel-Mechanik wie beim n8n-Provider-Streaming) wird zwischen Tool-Aufrufen geprüft und stoppt bei Verbindungsabbruch/Zeitlimit weitere Arbeit — die Antwort entsteht dann aus dem bereits Vorhandenen. Interne Prompts/Reasoning erreichen weder Stream noch Trace, siehe `AgentTrace.plan`s eigene Beschreibung in [`contracts/internal-chat.md`](../../contracts/internal-chat.md). `POST /internal/chat/stream` führt die komplette Pipeline innerhalb der bereits offenen Stream-Phase aus (Thread+Queue+Keepalive, wie beim n8n-Provider), `POST /internal/chat` synchron/blockierend.

#### Fortschritts-Ereignisse (`status`, Rollout-Plan "Schritt 4 -- Administration und Streaming")

Für einen streamenden Agentenmodus-Turn (Single-Subagent UND Graph-Pfad) gibt die Pipeline zusätzlich zu `trace`/`delta`/`sources`/`done`/`error` ein neues `status`-Ereignis aus — eine kurze, fest auf dieser Seite gebaute deutsche Fortschrittszeile pro Phase (`planning`/`researching`/`merging`/`answering`), z. B. `"IT Support wird durchsucht"` oder `"Antwort wird formuliert"`. Der Single-Subagent-Pfad (`_stream_deferred_agent`) meldet Recherche-Start/-Ende sowie den Beginn der Antwortformulierung; der Graph-Pfad (`agent_graph.py`s `_plan`/`_research`/`_merge`/`_answer`) meldet zusätzlich Planung und jede Merge-Runde, über denselben `progress_cb`-Mechanismus, den jeder Node bereits für `cancel: threading.Event` kennt. `answer` streamt im Graph-Pfad jetzt außerdem echte Deltas (`llm_service.iter_chat_stream` über dieselbe Queue) statt, wie im vorherigen Rollout-Schritt dokumentiert, einen einzigen blockierenden `chat()`-Aufruf danach in Stücke zu zerlegen. `status` trägt nie einen Prompt, eine Suchanfrage, einen Token oder Reasoning — nur `stage`/`agent_id`/`agent_name`/`state` plus die fertige `message`. Der blockierende `POST /internal/chat`-Pfad hat keine Verwendung für `progress_cb` (`None`, No-Op) und bleibt dadurch unverändert. Vertrag in [`contracts/internal-chat.md`](../../contracts/internal-chat.md).
...
**In Arbeit (Phase 4)** — Service-Skeleton (Config, Bot-YAML-Schema samt Validierung, Service-Token-Auth, `GET /health`/`GET /internal/bots`) steht, ebenso Intent-Router (`ROUTER_MODE=rules|llm`), LLM-Provider-Abstraktion (`fake`/`openai`) und die Weave-Retrieval-Anbindung als eigenständige, unit-getestete Module. `POST /internal/chat` (und dessen Streaming-Gegenstück `POST /internal/chat/stream`) ist jetzt die vollständige Pipeline (`backend/app/services/chat.py`): Bot-Lookup, Permissions, Intent-Routing, Retrieval mit nummeriertem Kontextblock, LLM-Aufruf und Response-Guard, End-to-End getestet (`backend/tests/test_chat_e2e.py`, `backend/tests/test_chat_stream.py`) mit `FakeLLM` und gemocktem Weave-Retrieval-HTTP. Vertrag in [`contracts/internal-chat.md`](../../contracts/internal-chat.md). n8n als zweiter, agentenfähiger Bot-Provider (Delegations-Token, SSRF-geprüfte Webhook-Anbindung) ist ebenfalls implementiert und getestet — Vertrag in [`contracts/n8n-flow.md`](../../contracts/n8n-flow.md); konkrete n8n-Flows werden außerhalb von Weave betrieben; die Token-Prüfung in `services/tools` ist Teil dieses Monorepos. Ein dritter Modus, der Agentenmodus (`agent.enabled`, `backend/app/services/agents.py`), erlaubt einem direkt-LLM-Bot gebundene `search_knowledge`-Tool-Aufrufe über konfigurierte Subagenten statt einer einzigen direkten Wissenssuche — LLM-Provider-Abstraktion (`app/services/llm.py`) und Bot-Schema (`app/schemas/bot.py`) unterstützen das für `fake` und (bei entsprechend deklariertem `model.supports_tools`) `openai`. Mit mehr als einem konfigurierten Subagenten übernimmt seit Rollout-Schritt 3 eine LangGraph-Orchestrator-Worker-Pipeline (`backend/app/services/agent_graph.py`, Abschnitt "Agentenmodus (LangGraph)" oben) das parallele Fan-out, Zusammenführen und ggf. Nachrecherche mehrerer Subagenten. Offen: echte LLM-Provider-Anbindung in Produktion (bislang nur `fake` in Tests/lokaler Entwicklung durchgespielt), Tool-Aufrufe für `action`/`complex` bei einem NICHT-n8n-, NICHT-Agentenmodus-Bot, Dokumentverarbeitung für `document` bei einem solchen Bot (alle drei antworten dort weiterhin mit einem Hinweis auf eine spätere Version), sowie technische Identitaeten/eine eigene MCP-Bereitstellung fuer Subagenten (laut Rollout-Plan bewusst auf einen spaeteren Schritt verschoben). Seit Rollout-Schritt 4 implementiert: Fortschritts-Events im Chat waehrend einer laufenden Agentenmodus-Recherche (`status`-Ereignis, Abschnitt "Fortschritts-Ereignisse" oben) und eine Admin-Oberfläche für den Agentenmodus (Weave-Ingest Bot-Editor: Subagenten, Collections, Filter, Grenzen). Noch offen aus Schritt 4: der zentrale Chat-Provider (Weave-Ingest) deklariert `supports_tools` noch nicht selbst -- Runtime liest das Feld bereits (`ChatProviderSnapshot.supports_tools`, Default `false`, sicherer Fallback auf RAG), aber die Ingest-seitige Spalte/Migration dafür ist noch nicht angelegt.
