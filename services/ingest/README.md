...
A **collection** is a named batch-upload target with its own storage path, department/email defaults, and an optional password — the same thing behind the upload wizard's "collection" mode and `POST /collections/{id}/upload` / `/start`. Every collection now also carries a **slug** (a stable, lowercase-hyphen identifier — auto-derived from its name if you don't set one) and a **name**, `description`, `visibility`, `read_teams`, and `read_users`.

A knowledge space is shareable for chat use three ways: **public** (`visibility: "public"` — everyone may read it), specific **teams** (`read_teams`, a list of team names), and specific **people** (`read_users`, a list of Weave-Ingest user ids as strings). `visibility` is the single authoritative public/restricted flag: `"public"` means every user may read the collection's documents regardless of `read_teams`/`read_users`; `"restricted"` means only `read_teams`/`read_users` (plus, within this service, the collection's own owner/managers/admins) may — a restricted collection with neither teams nor users configured is readable by nobody else (fail closed). `POST /collections` defaults `visibility` to `"public"` when both `read_teams` and `read_users` are omitted, else `"restricted"` (backward compatible with pre-`visibility` callers). A team member with the `member` membership role in one of `read_teams`, or any user listed in `read_users`, may also upload and process documents for the collection; a `reader` team role may only read. Collection settings and deletion remain restricted to the owner or an administrator. `PATCH /collections/{id}` validates every team name and user id in `read_teams`/`read_users` against real teams/users (422 on an unknown entry) and de-duplicates both lists; `POST /collections` does not validate them (same laissez-faire acceptance `read_teams` has always had on create).
...
- `POST /api/v1/collections` — create (`name`, optional `slug`, `description`, `visibility`, `read_teams`, `read_users`, plus the existing `email`/`department`/`folder`/`subfolder`/`password`)
- `GET /api/v1/collections` — list collections visible to the caller (same own/team/admin visibility rule as `GET /jobs`)
- `PATCH /api/v1/collections/{id}` — rename or change the description/`visibility`/`read_teams`/`read_users` (owner or admin; the stable slug never changes). Also queues the internal [`collection.updated`](../../contracts/events/collection.updated.md) freshness notification for Weave-Knowledge.
...
- `GET /api/v1/collections/{id}` — detail, including the ids of jobs uploaded into it, `visibility`, `read_teams`, `read_users`, and `read_user_details` (`[{id, username, display_name, team}]`, the person-picker's resolved display shape of `read_users` — a stale id whose account was deleted is silently dropped, never email or any other personal field)
- `GET /api/v1/collections/registry` — **admin-only.** `{"items": [{slug, name, description, visibility, read_teams, read_users}, ...]}` for every collection, unfiltered by caller visibility (same `{items: [...]}` envelope every other list endpoint here uses). This is the sync endpoint **Weave-Knowledge** polls to populate its own `collections` registry table, which **Weave-Retrieval** then reads to decide which collections a caller may search. It intentionally returns no document content or job data — identity/ACL metadata only, but that ACL metadata is the complete cross-team/cross-person access map of the system, so a non-admin caller gets 403. **The account whose token Weave-Knowledge uses for this sync must be an admin.**
- `GET /api/v1/directory/users?q=<text>&limit=<1..50>` / `GET /api/v1/directory/teams` — the person/team picker behind `read_users`/`read_teams`: case-insensitive search over active users' username/team, and every team with its member count. Allowed for admins and for anyone who can manage at least one collection (403 otherwise); never returns email or any other personal field, and `display_name` is always `null` today (this service has no per-user display name field yet).

Rather than relying solely on Weave-Knowledge's periodic poll against the endpoint above, `POST /api/v1/collections` and `PATCH /api/v1/collections/{id}` also queue a dedicated, signed [`collection.updated`](../../contracts/events/collection.updated.md) notification to Weave-Knowledge. Its payload contains only `event`, `timestamp`, and `slug`; it deliberately carries no ACL data (`visibility`/`read_teams`/`read_users`). Knowledge responds by re-pulling the admin-protected registry. This internal notification is not exposed through user-managed webhook subscriptions and is never sent to n8n.
