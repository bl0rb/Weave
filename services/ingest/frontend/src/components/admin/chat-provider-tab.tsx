...
export function ChatProviderTab() {
  ...
  return (
    <SectionCard
      title={t('admin.chatProvider.title')}
      description={t('admin.chatProvider.description')}
    >
      ...
      {loading || !config ? <LoadingState label={t('admin.chatProvider.loading')} /> : (
        <div className="space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-emerald-50 px-4 py-3">
            <Toggle checked={config.enabled} onChange={enabled => setConfig({ ...config, enabled })} label={t('admin.chatProvider.toggle.enable')} />
