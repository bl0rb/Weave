...
OpenAI-kompatibler Embedding-Server auf CPU fuer `intfloat/multilingual-e5-small` -- ein neuer, eigenstaendiger Dienst in diesem Monorepo (`services/embeddings/`, neben `services/tools/`, `services/chat/` und `services/reranker/`), mit eigenem `.venv` und eigenem Deployment. Weave-Knowledge bettet damit Chunks ein (`EMBEDDING_PROVIDER=openai`, siehe dessen `backend/app/services/embeddings.py`), Weave-Retrieval bettet damit Suchanfragen ein -- beide sprechen exakt dasselbe `/v1/embeddings` wie gegen die echte OpenAI-API.
