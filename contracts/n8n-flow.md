...
**Seit Vertrag-Version 2 kann n8n selbst inkrementell antworten**, statt wie in v1 nur blockierend in einem Stueck. Ob ein Bot das nutzt, steuert `bot.n8n.streaming` (`backend/app/schemas/bot.py`s `N8nConfig`) — die Einstellung entscheidet nur, WIE Weave-Runtime den Webhook aufruft (`run_flow` vs. `run_flow_stream`, `backend/app/services/n8n_client.py`), NICHT ob `POST /internal/chat/stream` selbst streamt: das tut es immer, mit oder ohne n8n-Streaming, unveraendert seit v1 (`trace` → `delta`* → `sources`+`done`/`error`).

### Request-Erweiterung (additiv, `bot.n8n.streaming: true`)

`run_flow_stream` sendet denselben Body wie `run_flow` (Abschnitt "Request" oben) mit EINEM zusaetzlichen Feld `"stream": true` — die Signatur wird wie gewohnt ueber die resultierenden kanonischen Bytes berechnet, es gibt keinen separaten Signierweg. Zusaetzlich setzt `run_flow_stream` einen erweiterten `Accept`-Header: `text/event-stream, application/json-lines, application/x-ndjson, application/json` — n8n (bzw. der jeweilige Trigger-Node) waehlt daraus sein bevorzugtes Antwortformat; Weave-Runtime erkennt das tatsaechlich gelieferte Format am `Content-Type` der Antwort (siehe unten), nicht an dem, was es angefragt hat.

### Response-Formate (unterschieden nach `Content-Type`)

n8n kann auf drei Arten antworten; `run_flow_stream` liest den Response-Koerper inkrementell (`httpx`, `stream=True`) und behandelt ihn je nach `Content-Type`:

1. **`text/event-stream`** — Weave-Runtimes EIGENER SSE-Vertrag, `Content-Type` ausschliesslich fuer diesen Fall zu verwenden (kein n8n-Eigenformat): Zeilen der Form `data: {json}\n\n`, jedes JSON-Objekt hat ein `type`-Feld:
   | `type` | Weitere Felder | Bedeutung |
   |---|---|---|
   | `delta` | `text` (`string`) | Ein Antwort-Fragment; mehrere `delta`-Frames werden aneinandergehaengt. |
   | `sources` | `sources` (`list[Source]`, Schema wie im Abschnitt "Response" oben) | Quellen fuer diesen Turn — hoechstens einmal, vor `done`. |
   | `done` | — | Erfolgreiches Ende des Flows. |
   | `error` | `detail` (`string`) | Der Flow ist fehlgeschlagen; `detail` ist eine fuer den Endnutzer sichere Kurzbeschreibung (keine internen URLs/Secrets). |
   Ein n8n-Flow, der dieses Format liefert, spricht also direkt denselben Frame-Vertrag, den `POST /internal/chat/stream` seinerseits nach aussen zeigt (Abschnitt "SSE-Frames" in `contracts/internal-chat.md`) — Weave-Runtime reicht `delta`/`sources`/`error` inhaltlich weiter, angewendet werden lediglich Quellen-Filterung und Guard wie unten beschrieben.
2. **`application/json-lines`, `application/x-ndjson`, `application/jsonl`, oder `text/plain` mit einem JSON-Objekt pro Zeile** — n8ns EIGENER "Streaming response"-Modus (Webhook-/Respond-to-Webhook-/Chat-Trigger-Node mit Response-Mode "Streaming"). n8ns oeffentliche Dokumentation legt das exakte Chunk-Schema nicht offen (Stand dieses Vertrags); `run_flow_stream` parst deshalb TOLERANT statt gegen ein hartkodiertes Schema: jedes Objekt mit einem String-Feld `content` UND `type` in `{item, chunk}` gilt als `delta` mit `text = content`; `type` in `{end, done, complete}` beendet den Flow erfolgreich, OHNE Quellen (n8ns natives Format kennt keinen Quellen-Kanal); `type == error` (mit `content` als Detailtext) wird zu `error`; jeder andere `type` wird ignoriert. **Vor produktivem Einsatz gegen eine konkrete n8n-Instanz pruefen** (Network-Tab einer echten "Streaming response"-Ausfuehrung oder n8ns Quellcode), ob deren tatsaechliche Feldnamen von dieser toleranten Annahme abweichen — dieser Abschnitt beschreibt den Fallback-Parser, keine verifizierte n8n-Spezifikation.
3. **`application/json` (n8n antwortet trotz `bot.n8n.streaming: true` nicht-inkrementell)** — wird exakt wie eine `run_flow`-Antwort in v1 geparst (`{answer, sources}`, Abschnitt "Response" oben) und als GENAU EIN `delta`-Ereignis plus `sources`+`done` emittiert, identisch zur v1-Ein-Delta-Behandlung. Ein Operator, der `streaming: true` setzt, bevor der eigentliche Flow selbst inkrementell antwortet, bekommt also weiterhin eine funktionierende, nur nicht-inkrementelle Antwort — kein Bruch.

### Quellen und Guard bei n8n-eigenem Streaming

Fuer Bots mit `bot.guard.require_sources: true` puffert Weave-Runtime ALLE `delta`-Fragmente, bis `sources`/`done` eintrifft — es wird also NIE ein Teiltext ausgeliefert, der anschliessend durch den Guard verworfen werden muesste. Erst nach `_filter_n8n_sources_by_scope` (Abschnitt "Quellen sind eine Behauptung, keine Berechtigung" oben) und der Guard-Pruefung wird entweder `bot.guard.no_context_reply` (Guard ausgeloest) oder der gepufferte Text als EIN `delta` plus die gefilterten `sources`+`done` gestreamt. Bots OHNE `require_sources` leiten `delta`-Fragmente dagegen sofort inkrementell weiter, sobald sie eintreffen; erst die abschliessenden `sources` werden weiterhin durch `_filter_n8n_sources_by_scope` gefiltert. Fuer den nicht-inkrementellen Fall (Content-Type `application/json`, oder `bot.n8n.streaming: false`) gilt unveraendert die v1-Ein-Delta-Behandlung.

### Idle-Timeout statt Gesamt-Zeitlimit

`bot.n8n.timeout_seconds` ist bei `run_flow_stream` ein LEERLAUF-Timeout zwischen zwei empfangenen Chunks (httpx' `read`-Timeout greift bereits pro Socket-Read), kein Gesamtzeitlimit fuer den Flow — ein Flow, der regelmaessig (schneller als `timeout_seconds`) weitere Chunks liefert, kann beliebig lange laufen. Waehrend Weave-Runtime auf den naechsten n8n-Chunk wartet, haelt es die eigene SSE-Antwort ueber Keepalive-Kommentarzeilen offen (`contracts/internal-chat.md`, Abschnitt "Keepalive"); ein Ausbleiben von n8n-Chunks fuer laenger als `timeout_seconds` fuehrt weiterhin zu `N8nUnavailable`/einem terminalen `error`-Frame, exakt wie ein Timeout bei `run_flow` in v1.

### Fehlerabbildung

Nicht-2xx-Antworten und Transportfehler (Verbindungsabbruch, DNS, TLS) werden bei `run_flow_stream` auf dieselben Klassen abgebildet wie bei `run_flow` (`N8nUnavailable`/`N8nError`, Abschnitt "Fehlerbilder" oben) — der einzige Unterschied ist, WANN der Fehler auftreten kann: da der Webhook-Aufruf jetzt WAEHREND der bereits offenen SSE-Antwort laeuft (siehe `contracts/internal-chat.md`), kann ein solcher Fehler nicht mehr als HTTP-Statuscode VOR Stream-Beginn erscheinen, sondern wird als terminales `{type: "error", detail}`-Ereignis im Stream ausgeliefert. `detail` ist dabei bewusst NICHT die `N8nUnavailable`/`N8nError`-Meldung (die den `webhook_url` und bis zu 200-500 rohe Zeichen der n8n-Antwort nennt), sondern ein generischer deutscher Text (nicht erreichbar/Timeout · ungueltige Antwort · unerwartet fehlgeschlagen); die vollstaendige Meldung wird in Weave-Runtime geloggt. Auf dem Stream-Pfad reicht Weave-API `detail` unveraendert bis zum Browser durch — deshalb gilt hier die Garantie: `detail` enthaelt nie `webhook_url`, interne Adressen oder Antwortfragmente. Nur ein vom Flow selbst gemeldeter Fehlertext (`{type: "error"}`) wird — wie ein Antworttext, auf 500 Zeichen gekuerzt — durchgereicht.
...
- **v2 (2026-09-18):** n8n-eigenes inkrementelles Antworten (`bot.n8n.streaming: true`, `run_flow_stream`) — additiv im Request (`stream: true`-Feld, erweiterter `Accept`-Header), aber ein neues Antwortformat auf der n8n-Seite (SSE, JSON-Lines/NDJSON, oder weiterhin `application/json`), deshalb Versionserhoehung statt rein additiver Aenderung. Abschnitt "Streaming" komplett neu gefasst: die drei Content-Type-Faelle, Pufferung fuer `require_sources`-Bots, Leerlauf- statt Gesamt-Timeout-Semantik, Fehlerabbildung auf ein terminales `error`-Frame statt eines Vor-Stream-HTTP-Fehlers. Siehe `contracts/internal-chat.md` fuer die Keepalive-Kommentarzeilen, die waehrend eines laufenden n8n-Aufrufs die SSE-Verbindung offen halten.
