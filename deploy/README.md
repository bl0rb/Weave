...
- `EMBEDDING_REQUEST_TIMEOUT_SECONDS` (180), `EMBEDDING_MAX_ATTEMPTS` (6) – `weave-knowledge` (+worker) only: client read timeout and retry ceiling for a single call into the embedding provider (incident 2026-09-22 — see `EMBEDDINGS_MAX_CONCURRENT_REQUESTS` below); the timeout must stay above `weave-embeddings`' own `EMBEDDINGS_QUEUE_TIMEOUT_SECONDS`
- `CELERY_WORKER_CONCURRENCY` (2) – Celery worker process concurrency for `weave-knowledge-worker` only (siehe [„Indexierung unter Last"](#indexierung-unter-last) unten). Weave-Ingest's own OCR worker knob below is deliberately named `INGEST_WORKER_CONCURRENCY` instead, so the two cannot collide once docker compose resolves `${VAR:-default}` from the one shared variable pool this stack renders into `deploy/.env`
...
- `EMBEDDINGS_MAX_CONCURRENT_REQUESTS` (1), `EMBEDDINGS_QUEUE_TIMEOUT_SECONDS` (90) – `weave-embeddings` only: bounds concurrent `encoder.encode()` calls so a burst of Knowledge indexing cannot OOM this container again (incident 2026-09-22); once more requests wait than fit in the timeout, the service answers `503` with `Retry-After: 5`
...
- Lange n8n-Streaming-Turns halten den SSE-Body ggf. sehr lange offen: Runtime und API senden dabei alle paar Sekunden (aktuell ein fester Default von 5s, noch nicht als Umgebungsvariable exponiert) eine Keepalive-Zeile, damit kein Proxy dazwischen abbricht. Der ingress-nginx-Controller braucht dafür `proxy-buffering: "off"` sowie `proxy-read-timeout`/`proxy-send-timeout: "3600"` (siehe `ingress.chat.annotations` / `ingress.api.annotations` in `deploy/charts/weave/values.yaml`); die tatsächliche Obergrenze bleibt der Leerlauf-Timeout des jeweiligen n8n-Bots (`timeout_seconds`, siehe `contracts/n8n-flow.md`).
...
- `WORKER_MEMORY_LIMIT` (3g), `WORKER_CPUS` (2.0), `INGEST_WORKER_CONCURRENCY` (1), `CELERY_PREFETCH_MULTIPLIER` (1), `CELERY_MAX_TASKS_PER_CHILD` (5) – Weave-Ingest OCR worker; rendered into the container as `CELERY_WORKER_CONCURRENCY` (matching `worker.Dockerfile`'s own default fallback), just under a different externally-facing variable name — see the `CELERY_WORKER_CONCURRENCY` entry above
...
## Indexierung unter Last

Incident 2026-09-22: viele gleichzeitig ausgelieferte Releases ließen den unbegrenzten Weave-Knowledge-Worker Dutzende parallele Embedding-Requests gegen `weave-embeddings` feuern, das daraufhin OOMKilled wurde — Dokumente blieben dauerhaft `failed`. `CELERY_WORKER_CONCURRENCY` (2) begrenzt jetzt die Worker-Nebenläufigkeit, `EMBEDDINGS_MAX_CONCURRENT_REQUESTS` (1) / `EMBEDDINGS_QUEUE_TIMEOUT_SECONDS` (90) serialisieren `weave-embeddings` selbst (`503` + `Retry-After` statt OOM), und `EMBEDDING_REQUEST_TIMEOUT_SECONDS` (180) / `EMBEDDING_MAX_ATTEMPTS` (6, Backoff bis 60min) lassen Retries einen Pod-Neustart überstehen statt sofort dauerhaft auf `failed` zu setzen. Alle fünf Werte sind oben unter Environment Variables dokumentiert.
